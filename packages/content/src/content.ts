...
// Shared by frontmatter values and directive configs. No expressions, no code.
// ---------------------------------------------------------------------------

type PValue = string | number | boolean | PValue[] | { [k: string]: PValue };
...
function parseScalar(s: string): string | number {
  ...
  return t; // 'on'/'off' and every other bare word stay strings
}

function parseValueString(raw: string): PValue {
  const s = raw.trim();
  if (s.startsWith('[')) {
...
function hasExecutable(text: string): boolean {
...
// Diagnostics helper
// ---------------------------------------------------------------------------
function diag(code: DiagCode, severity: Severity, message: string, loc: Loc): Diagnostic {
  return { code, severity, message, loc };
}
function loc(line: number, startCol: number, endCol: number): Loc {
