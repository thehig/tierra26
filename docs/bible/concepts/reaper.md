...
## Simple
The {reaper} is what keeps the {soup world} from overflowing: it removes creatures to make room. Creatures
that make errors get pushed toward the front of the line and die sooner; creatures that
successfully make {daughter babies} get nudged back and live a little longer.

## Advanced
The {reaper} is a queue where index 0 is the next creature to die. It works with the {flag-e} flag and the
{soup}'s fullness:
...
Note: the {flag-e} flag being cleared later (e.g. by executing a `nop`) does **not** reverse the
