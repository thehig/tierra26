...
## Simple
A {label} is a friendly name for a spot in your code. You put a label down as a {label landmark}, and jump
or find *to that name*. Behind the scenes the machine turns each label into a unique little
{template signpost} pattern so it knows exactly which spot you meant.
...
- serializes a label **definition** as its pattern's `nop0`/`nop1` bytes (a {label landmark});
...
The disassembler reconstructs labels in reverse, naming inferred {label landmarks} `label1`, `label2`, …
in defining-byte order.

A **spacer** (a real instruction) is required between two adjacent {label landmarks}, or their nop runs
