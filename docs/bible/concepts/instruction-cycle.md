...
points to, work out what instruction it is, do it, then move the {reading-head} on. The {soup world} gives
...
2. **Decode** — reset the shared decode scratch (no leakage between ops), then, keyed on the
   ...
   the {mutation} seam), source/destination addresses, and — for target-taking ops — measure the own
