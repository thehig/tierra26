...
you write a little pattern of {template signposts} right after the instruction, and the machine goes looking
...
`mal` and `divide` are control verbs but take **no** target — `mal` reads its size from register {register-c}
...
the address into {register-a} and the size into {register-c}. On a miss within the limit: the {flag-e} flag is raised.
