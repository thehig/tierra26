...
## Simple
A {template} is a little pattern spelled out with the two {template signpost} bits, `mark-0` and `mark-1`. It
is how the machine finds places without using address numbers. To find a {label landmark}, the machine
...
is the template length. A miss within `searchLimit` raises the {flag-e} flag.
