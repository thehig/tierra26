...
## Simple
{age Age} counts how many {instruction-cycle ticks} a creature has been alive, starting from the moment it
...
## Advanced
{age Age} is derived, not stored: it is `cycles - bornAtCycle`, so it is measured in
instruction cycles rather than reproductions.

{age Age} is **not** the {reaper}'s ordering key. The reaper keeps its own queue, and a
creature's place in it moves for reasons other than time: raising the {flag-e} flag
