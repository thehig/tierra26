...
- **Z** (zero) turns on when the last sum came out to zero.
- **S** (sign) turns on when the last sum came out negative.
- **E** (error) turns on when something went wrong — like a jump that found nothing, or trying to
...
**S and Z** are set together by `applyFlags(v)`: `S = (v < 0)`, `Z = (v == 0)`. The ops that call
...
but **do not** set S/Z: `pop*` (the pop handler skips `applyFlags`) and the addressing ops `adr*`
(which write A/C but set no S/Z).

**E** is the error flag. It is set true **only** by `raiseE`, which also increments the creature's
...
**E is cleared** (along with S and Z) by the `nop` handler — so executing a `nop0`/`nop1` resets
