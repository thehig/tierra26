...
## Simple
A {daughter} is the baby a creature builds. Making one has three steps: ask the {soup world} for room
...
3. **`divide`** — if there is a {daughter} and it is at least 70% written
   ...
   daughter fields clear, and the mother moves **down** the reaper queue. Otherwise it raises {flag-e}.
