...
## Simple
The other kind of {template signpost}. `mark-1` is the opposite bit to `mark-0`. You spell a {label landmark} by
...
- Reads: nothing.
- Writes: nothing.
- Flags: clears {flag-e}, {flag-s} and {flag-z}.

## Gotchas
- Flipping one bit of a {label landmark} (a `mark-0`↔`mark-1` change) changes the pattern a jump aims
  for, so the matching jump will miss or land elsewhere.
- Adjacent {label landmarks} blur into one longer {template}; separate them with a real instruction.
- Executing `nop1` clears the {flag-e} flag but does not reverse an error already recorded.
