...
## Simple
Jumps to a matching signpost **behind** the reader — the usual way to loop back and repeat.
...
suppressed; on a miss within the search limit it raises the **E flag**.
...
## Gotchas
- The landmark must be **behind** the jump. A matching pattern that sits ahead is not found.
- A loop with no exit condition (e.g. no `ifz`/`decC` guard) repeats forever.
- Empty template or out-of-range landmark → miss → E.
