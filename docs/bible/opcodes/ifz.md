...
## Simple
A gate for the very next line. If the counting box (C) is empty (zero), the next line runs. If C
holds any other number, the next line is skipped completely.

## Advanced
Looks at C. If `C != 0` it sets the IP advance to 2, so the reading head steps over the following
...
- The sense is "run next line **only if C is zero**"; any non-zero C skips it.
- `ifz` checks the register value directly; it does **not** read the Z flag.
