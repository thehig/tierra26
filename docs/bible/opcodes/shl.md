...
## Simple
Doubles the number in the counting box ({register-c}) by sliding all its bits up one place. Do it three
...
push a 1 into bit 31 becomes **negative**, which sets the {flag-s} flag. {flag-s}/{flag-z} are set from the result.
...
## Gotchas
- Doubling grows a number fast; repeated shifts can overflow into a negative value ({flag-s} set).
