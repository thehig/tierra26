...
## Advanced
`IP := pop()` (wrapped to the {soup}: `IP = ad(pop())`). It pops one value off the ring {save-pile stack} and
sets the {reading-head} to that address, suppressing the normal advance. It sets no flags and, notably,
**never raises** {flag-e} — even with an empty pile it simply pops a stale slot and jumps there.
...
## Gotchas
- `ret` only works if a `call` (or a matching `push`) put a real return address on the {save-pile pile}
