...
## Simple
A signpost. `mark-0` does no work on its own — it just sits in your code as a landmark that
jumps and searches can aim for. A row of `mark-0` and `mark-1` signposts spells out a pattern,
...
- Flags: clears **E**, **S** and **Z** (the shared `nop` handler sets all three to false).

## Gotchas
- Two signposts with the same bit-pattern are ambiguous — a search lands on the nearest one.
- Signposts do nothing by themselves; something has to jump to or search for the pattern.
- Because `nop0` clears the E flag, dropping one into a hot loop quietly wipes the error flag —
...
- Two adjacent nop runs merge into one longer template. Keep a real instruction between separate
  landmarks (see [template](../concepts/template.md)).
