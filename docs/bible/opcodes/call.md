...
## Simple
Runs a helper routine and remembers where to come back to. The {reading-head reader} jumps to the matching
{template signpost}, and later a `return` brings it right back to the line after the call.
...
(`ad(IP + 1 + templateLen)`), and searches **outward** for the complement. On a hit it **pushes** that return address onto the {save-pile}, sets `IP` to just past the matched target template, and
suppresses the normal advance. On a miss within the limit it raises the {flag-e} flag and pushes
nothing.
...
- Every `call` needs a matching `ret`; otherwise the saved return address is left on the {save-pile pile} and
