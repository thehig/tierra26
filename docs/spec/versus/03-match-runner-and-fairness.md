...
interface MatchDescriptor {                                        // the reproducible recipe (00 §5)
  scenario: Scenario; seed: number;
  players: { founderId: number; genome: string }[];               // ordered
...
- **`MatchDescriptor`** — the full, reproducible recipe (scenario+seed+ordered players+placement+
  ...
  `RunDescriptor` (adds players/placement/threshold). Serializes to a `RunLink` (UI [07]).
...
- **UI [07] Shell** — `MatchDescriptor` ↔ `RunLink` share/deep-link; scoreboard renders
  `standings$`.
...
- **RUNNER-006** `runMatch(desc)` reproduces identical live standings + result for any viewer
  (VSINV-DET).
- **RUNNER-007** A `MatchDescriptor` round-trips to/from a `RunLink` (share/deep-link).
